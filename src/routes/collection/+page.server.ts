...
async function fetchSheetData() {
    ...
    return {
        firstRange: result.data.valueRanges[0].values,
        secondRange: result.data.valueRanges[1].values,
    };
}
...
export const actions = {
    markPresent: async (event) => {
        if (result.data.valueRanges && result.data.valueRanges.length > 1) {
            const { firstRange, secondRange } = await fetchSheetData();
            if (
                secondRange.values &&
